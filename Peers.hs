...
-- makePeer initializes the Peer data type for each peer
...
-- It forks a thread for a new connection corresponding to each peer
...
-- This function starts communication with peer by completing the handshake protocol
-- then it runs the two IO functions i.e., listenPeer and talkWithPeer concurrently
-- where all the message exchange is happening.
...
-- This function tells what should be done when we receive a particular message.
...
listenPeer tor peer = forever $ do
						...
						case msg of
							...
							Piece pId os content -> do 								-- This message actually conatins the data of the required file
											let pieceId = word32ToInt pId 			-- Thus we flush this data to the output file
...
-- It sends our response to peer after the peer's response
...
-- It tells the thread which piece to request and what will be the next piece to download.
...
-- It keeps on updating the incoming data to current stored data 
